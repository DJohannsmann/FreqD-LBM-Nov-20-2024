...
if __name__ == '__main__': 
    if len(sys.argv) == 2:
                ...
                for SPs['iPar2'],SPs[SPs['Par2str']] in enumerate(Pars2):
                    for SPs['iPar3'],SPs[SPs['Par3str']] in enumerate(Pars3):
                        ...
                        for SPs['iovt'],SPs['n'] in enumerate(ns):
                            ...
                            print('******************')
                            print('******************')
                            arguments.append(SPs.copy())

        before = time.time()
        pool = mpr.Pool(mpr.cpu_count()-1)
        pool.map_async(Single_Sim_3D.SingleSimulation,arguments)
        pool.close()
        pool.join()
        after = time.time()
        # try:
        ...
        #np.save(pfolder+'\\STOP', SPs, allow_pickle=True)
        print('Done! '+str(after - before)+' Press enter to exit.')
        input()            
